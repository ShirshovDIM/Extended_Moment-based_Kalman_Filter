...
StateInfo SimpleVehicleNKF::predict(const StateInfo& state_info,
    ...
    const Eigen::Vector2d& noise_variances)
{

    const auto state_mean = state_info.mean;
    auto state_cov = state_info.covariance;
    // Убедимся, что матрица симметрична
    state_cov = (state_cov + state_cov.transpose()) / 2.0;
    // Проверяем собственные значения и корректируем, если нужно
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigen_solver_input(state_cov);
    if (eigen_solver_input.eigenvalues().minCoeff() <= 0) {
        const double epsilon = 1e-6;
        state_cov += epsilon * Eigen::MatrixXd::Identity(state_cov.rows(), state_cov.cols());
    }
    ThreeDimensionalNormalDistribution dist(state_mean, state_cov);
    ...
    // Используем noise_variances вместо noise_map
    SimpleVehicleModel::SystemNoiseMoments system_noise_moments;
    system_noise_moments.wvPow1 = 0.0;  // Первый момент (среднее) = 0
    system_noise_moments.wvPow2 = noise_variances(0);  // Второй момент = дисперсия
    system_noise_moments.wuPow1 = 0.0;  // Первый момент (среднее) = 0
    system_noise_moments.wuPow2 = noise_variances(1);  // Второй момент = дисперсия
    system_noise_moments.cwuPow1 = 0.0;  // cos(wu) для гауссовского шума с нулевым средним
    system_noise_moments.swuPow1 = 0.0;  // sin(wu) для гауссовского шума с нулевым средним
    system_noise_moments.swuPow2 = 0.0;  // Второй момент sin(wu)
    system_noise_moments.cwuPow2 = 0.0;  // Второй момент cos(wu)
    system_noise_moments.cwuPow1_swuPow1 = 0.0;  // Кросс-момент cos(wu)sin(wu)
    ...
    // Инициализируем covariance нулями
    predicted_info.covariance.setZero();
    ...
    // Проверяем на NaN
    if (predicted_info.covariance.hasNaN()) {
        std::cout << "[Predict] Covariance contains NaN, resetting to identity matrix" << std::endl;
        predicted_info.covariance = Eigen::Matrix3d::Identity();
    }

    // Корректируем ковариацию перед возвратом
    predicted_info.covariance = (predicted_info.covariance + predicted_info.covariance.transpose()) / 2.0;
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigen_solver_output(predicted_info.covariance);
    if (eigen_solver_output.eigenvalues().minCoeff() <= 0) {
        const double epsilon = 1e-6;
        predicted_info.covariance += epsilon * Eigen::MatrixXd::Identity(predicted_info.covariance.rows(), predicted_info.covariance.cols());
        std::cout << "[Predict] Covariance was not positive definite, corrected with epsilon = " << epsilon << std::endl;
    }
    ...
    const Eigen::Vector2d& noise_variances)
{
    ...
    auto predicted_cov = state_info.covariance;
    // Убедимся, что матрица симметрична
    predicted_cov = (predicted_cov + predicted_cov.transpose()) / 2.0;
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigen_solver_input(predicted_cov);
    if (eigen_solver_input.eigenvalues().minCoeff() <= 0) {
        const double epsilon = 1e-6;
        predicted_cov += epsilon * Eigen::MatrixXd::Identity(predicted_cov.rows(), predicted_cov.cols());
    }
    ...
    observation_noise.wrPow1 = 0.0;  // Первый момент (среднее) = 0
    observation_noise.wrPow2 = noise_variances(0);  // Второй момент = дисперсия
    observation_noise.cwaPow1 = 0.0;  // cos(wa) для гауссовского шума с нулевым средним
    observation_noise.swaPow1 = 0.0;  // sin(wa) для гауссовского шума с нулевым средним
    observation_noise.cwaPow2 = 0.0;  // Второй момент cos(wa)
    observation_noise.swaPow2 = 0.0;  // Второй момент sin(wa)
    observation_noise.cwaPow1_swaPow1 = 0.0;  // Кросс-момент cos(wa)sin(wa)
    ...
    Eigen::MatrixXd state_observation_cov(3, 2);
    ...
    case SimpleVehicleNKF::InversionMethod::DIRECT:
        S_inv = S.inverse();
        std::cout << "[NKF] The DIRECT method is used (S.inverse())" << std::endl;
        break;
    case SimpleVehicleNKF::InversionMethod::BFGS:
        S_inv = invertBFGS(S);
        std::cout << "[NKF] The BFGS algorithm is used to reverse the matrix" << std::endl;
        break;
    case SimpleVehicleNKF::InversionMethod::DFP:
        S_inv = invertDFP(S);
        std::cout << "[NKF] The DFP algorithm is used to reverse the matrix" << std::endl;
        break;
    case SimpleVehicleNKF::InversionMethod::LBFGS:
        S_inv = invertLBFGS(S);
        std::cout << "[NKF] The LBFGS algorithm is used to reverse the matrix" << std::endl;
        break;
    default:
        S_inv = S.inverse();
        std::cout << "[NKF] Mode is not recognized; DIRECT is used" << std::endl;
    }
    ...
    // Проверяем на NaN
    if (updated_info.covariance.hasNaN()) {
        std::cout << "[Update] Covariance contains NaN, resetting to identity matrix" << std::endl;
        updated_info.covariance = Eigen::Matrix3d::Identity();
    }

    // Корректируем ковариацию перед возвратом
    updated_info.covariance = (updated_info.covariance + updated_info.covariance.transpose()) / 2.0;
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigen_solver_output(updated_info.covariance);
    if (eigen_solver_output.eigenvalues().minCoeff() <= 0) {
        const double epsilon = 1e-6;
        updated_info.covariance += epsilon * Eigen::MatrixXd::Identity(updated_info.covariance.rows(), updated_info.covariance.cols());
        std::cout << "[Update] Covariance was not positive definite, corrected with epsilon = " << epsilon << std::endl;
    }
